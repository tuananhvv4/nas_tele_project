...
<div class="space-y-6">
    <div class="flex items-center gap-3">
        ...
        <h1 class="text-2xl font-bold text-gray-800">Thêm sản phẩm</h1>
    </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-x-6">
                    <div class="form-group">
                        ...
                        <select name="category_id" class="form-select">
                            ...
                            <option value="<?= $cat['id'] ?>" <?= \Core\View::old('category_id') == $cat['id'] ? 'selected' : '' ?>><?= htmlspecialchars($cat['name']) ?></option>
                            <?php endforeach; ?>
                        </select>
                    </div>
                    <div class="form-group">
                        <label class="form-label">SKU</label>
                        <input type="text" name="sku" class="form-input" value="<?= \Core\View::old('sku') ?>">
                    </div>
                </div>

                <div class="form-group">
                    <label class="form-label">Mô tả</label>
                    <textarea name="description" class="form-textarea" rows="4"><?= htmlspecialchars(\Core\View::old('description')) ?></textarea>
                </div>

                <div class="grid grid-cols-1 md:grid-cols-3 gap-x-6">
                    <div class="form-group">
                        <label class="form-label">Giá <span class="text-red-500">*</span></label>
                        <input type="number" name="price" class="form-input" value="<?= \Core\View::old('price') ?>" min="0" step="1000" required>
                    </div>
                    <div class="form-group">
                        <label class="form-label">Giá khuyến mãi</label>
                        <input type="number" name="sale_price" class="form-input" value="<?= \Core\View::old('sale_price') ?>" min="0" step="1000">
                    </div>
                    <div class="form-group">
                        <label class="form-label">Tồn kho <span class="text-gray-400 text-xs">(-1 = không giới hạn)</span></label>
                        <input type="number" name="stock" class="form-input" value="<?= \Core\View::old('stock', '-1') ?>">
                    </div>
                </div>
